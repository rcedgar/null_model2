...
int alignStructure_rce(const char *qaa, const char *taa, StructureSmithWaterman & structureSmithWaterman,
                   StructureSmithWaterman & reverseStructureSmithWaterman,
                   Sequence & tSeqAA, Sequence & tSeq3Di,
                   unsigned int querySeqLen, unsigned int targetSeqLen,
                   EvalueNeuralNet & evaluer, std::pair<double, double> muLambda,
    ...
    int fwdscore = int(align.score1); //@@RCE
    int diffscore = int(score); //@@RCE
    double pvalue = evaluer.computePvalue(score, muLambda.first, muLambda.second); //@@RCE
    align.evalue = evaluer.computeEvalueCorr(score, muLambda.first, muLambda.second);
    //if (hasLowerEvalue) {
    //    return -1;
    //}//@@RCE
    ...
    fprintf(f, "%.4g", align.evalue);
    fprintf(f, "\t%.4g", pvalue);
    fprintf(f, "\t%d", fwdscore);
    fprintf(f, "\t%d", diffscore);
    fprintf(f, "\t%d", int(tSeqAA.getDbKey()));
    fprintf(f, "\t%d", querySeqLen);
    fprintf(f, "\t%d", targetSeqLen);
    fprintf(f, "\t");
    for (int i = 0; i < 12; ++i)
        {
        char c = qaa[i];
        ...
        for (size_t id = 0; id < resultReader.getSize(); id++) {
            ...
            if(*data != '\0') {
                ...
                while (*data != '\0' && passedNum < par.maxAccept && rejected < par.maxRejected) {
                    ...
                    if(alignStructure_rce(querySeqAA, targetSeqAA, structureSmithWaterman, reverseStructureSmithWaterman,
                                      tSeqAA, tSeq3Di, querySeqLen, targetSeqLen,
